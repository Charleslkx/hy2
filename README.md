...
### 核心脚本文件
- **`main.sh`** - 🆕 统一管理脚本，智能 swap 管理 + 脚本选择菜单
- **`install.sh`** - 主安装脚本，八合一共存 + 增强功能
- **`v2ray.sh`** - V2Ray 专用安装脚本
...
- **✅ 智能冲突检测**: 避免重复配置和规则冲突
- **🆕 ✅ main.sh 统一管理**: 智能 swap 管理 + 脚本选择菜单
- **🆕 ✅ 智能内存检测**: 自动推荐最优 swap 配置
- **🆕 ✅ 环境兼容性检查**: 自动检测虚拟化环境支持
...
4. **🧹 完整清理**: 卸载时不留任何配置残余
5. **💡 智能管理**: 自动检测和避免配置冲突
6. **🆕 🎯 统一入口**: main.sh 提供一站式脚本管理
7. **🆕 🧠 智能配置**: 自动检测系统环境并推荐最优设置
8. **🆕 👥 用户友好**: 清晰的菜单界面和操作指导
